...
export interface HandoffContext {
  ...
  blockedItems?: string[];
  // Component 20: Memory write proposals (operator must confirm)
  memoryWriteProposals?: Array<{
    category: string;
    title: string;
    description: string;
    source: string;
...
${ctx.memoryWriteProposals && ctx.memoryWriteProposals.length > 0
  ? `The following memory items are proposed for retention. The operator must confirm which to keep:\n${ctx.memoryWriteProposals.map(p => `- **[${p.category}]** ${p.title}: ${p.description.slice(0, 100)}...`).join('\n')}`
  : 'No memory write proposals from this conversation.'}
